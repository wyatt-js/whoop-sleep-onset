...
The repository contains application code; AWS infrastructure must be provisioned separately. Build from source rather than using the old checked-in binaries.
...
3. Connect the table stream to the sync Lambda. Filter for inserted webhook rows (`PK` starts with `WHOOPUSER#`, `SK` starts with `WEBHOOK#`). Use parallelization factor 1 to preserve ordering for a user; configure finite retries and an on-failure destination so a bad record cannot block the stream indefinitely.
4. Give the API Lambda read/write access only to this table and index, plus `secretsmanager:GetSecretValue` for the `WSO` secret. Give the sync Lambda read/write/**DeleteItem** access to this table, read access to its stream, and access to the same secret. Both functions need CloudWatch logging and outbound HTTPS access.
5. Configure an API Gateway HTTP API with Lambda proxy payload format **2.0**, routing these paths to the API Lambda:
...
Register the same redirect URI in your WHOOP developer app. Set the webhook URL to `https://YOUR_API_HOST/webhook/whoop`, using **v2** webhooks. The app requests `offline`, `read:sleep`, `read:recovery`, `read:profile`, and `read:cycles` scopes. Webhooks are verified with the WHOOP client secret.
...
Configure the stream mapping with batches of one, three retries, a one-hour maximum record age, and an encrypted SQS failure destination. This prevents one malformed webhook from blocking later records and preserves failures for inspection.
